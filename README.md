...
Before opening voting, check the closed page and Finale event link on the deployed site, confirm the Twilio and Sheets settings, and run `npm run check:functions` and `npm run test:voting` locally. The voting tests mock SMS and storage; they do not send texts or write live ballots. Open voting with a separate `FINALE_VOTING_OPEN=true` deployment when ready. After opening, submit one authorized test ballot and confirm it appears in `Finale Votes`; that action sends a real SMS and is intentionally outside the local test suite.
