...
if (categoryPicker) {
  const chips = Array.from(categoryPicker.querySelectorAll("[data-category-chip]"));
  const detail = categoryPicker.querySelector("[data-category-detail]");
  const detailTitle = categoryPicker.querySelector("[data-category-detail-title]");
  const detailCopy = categoryPicker.querySelector("[data-category-detail-copy]");
  ...
  chips.forEach((chip) => {
    chip.addEventListener("click", () => {
      ...
      if (detailTitle) {
        detailTitle.textContent = chip.dataset.categoryTitle || chip.textContent.trim();
      }

      if (detailCopy) {
        detailCopy.textContent = chip.dataset.categoryCopy || "";
      }
